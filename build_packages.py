...
def packaged_config(key):
    return dict(config(key), strategy_memory=key != "wallet_control", external_predictions=key != "wallet_control", research_protocol=LATEST_VERSION, weather_hypotheses=[] if key=='wallet_control' else list(HYPOTHESES))
