...
sequenceDiagram
    ...
    B->>B: Compare identical scored cases and calendar-day bootstrap intervals
    F-->>D: Local aggregate report with coverage, scores, graph and cloud status
    Note over B,D: No historical books, wallet trades, full market blend or profit claim
```

The recent-window downloader is separate from the original August fixture downloader. Both use the bounded historical runner. Archives remain outside the live evidence index. The September 6–15 pilot contains five stations and keeps calibration in August 6–September 5. The initial cloud attempt was blocked by provider credits. After funding, fresh cloud runs use separate output directories and the same daily cap. The statistical baseline is retained for comparison. See [RECENT-HISTORY.md](RECENT-HISTORY.md) for reproducible commands, evidence paths and limits.

## Supplemental US market archive

```mermaid
sequenceDiagram
    participant C as Historical market collector
    participant P as Polymarket US public gateway
    participant A as Immutable market archive
    participant W as Weather outcome audit
    participant D as Dashboard report
    C->>P: Paginated closed climate contracts within bounded dates
    P-->>C: Contract IDs, exact station rules and weather-day intervals
    C->>C: Reject unsupported rules and duplicate inventory pages
    loop At most 600 contracts and 900 seconds
        C->>P: Thirty-day display-price history and current settled book
        P-->>C: Timestamped display prices and binary USD settlement
        C->>A: Preserve receipts, hashes, download times and missing-depth flags
    end
    A->>W: Match station, date, day interval and temperature bucket
    W->>W: Cross-check venue payout against final CLI high
    W-->>D: Coverage and settlement agreement only
    Note over A,D: Display prices lack historical quantity and trade identity
    Note over A,W: No fabricated fills or public-wallet signals, no RAG backdating
    Note over W,D: Market archive does not alter frozen weather model inputs
```
...
This isolated research pipeline reproduces the daily-temperature analysis in the supplied video. It has no connection to the four trading agents or frozen dashboard. Outcomes enter scoring after price-only signal timestamps are calculated; they never choose a favorite. Missing histories and non-binary/ambiguous outcomes are excluded and reported.

```mermaid
flowchart TD
    G[Gamma weather events and CLOB YES histories] --> C[Resumable public collector]
    U[US climate inventory and minute display histories] --> C
    V[US terminal settlement books] --> C
    C --> R[Compressed source receipts and normalized histories]
    R --> A[30-minute as-of price grid]
    A --> D[Six price-sum checks before 95-cent cutoff]
    D --> T[Retrospective start and sixth-check confirmation]
    R --> O[Verified single winning bucket]
    T --> S[Favorite accuracy and calibration scoring]
    O --> S
    S --> F[Platform and city figures, CSV and PDF]
```

Source tools: `tools/weather_crowd_study.py` and `tools/graph_weather_crowd.py`. [Full methodology](CROWD-CALIBRATION-STUDY.md). [Offline component figure](architecture/05-crowd-study.svg). Public read-only batching and network concurrency are data acquisition, not autonomous trading agents. The study uses no inference or paid calls.
